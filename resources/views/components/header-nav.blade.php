    ...
    <div class="hidden lg:flex items-center gap-10 justify-center flex-1">
        @foreach ($navItems as $item)
            @if (isset($item['dropdown']))
                ...
                <div x-data="{ open: false, selected: '{{ $activeSubLabel }}' }" x-init="selected = '{{ $activeSubLabel }}'" class="relative group">
            ...
            @else
                <a href="{{ $item['href'] }}"
                    class="transition font-bold
                      {{ ($item['href'] === '/' && request()->is('/')) ||
                      Str::startsWith(request()->path(), ltrim($item['href'], '/'))
...
">
    ...
    <div x-show="sidebarOpen" x-transition:enter="transition duration-300" x-transition:enter-start="-translate-x-full"
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <button @click="sidebarOpen = false" class="cursor-pointer"><i
                    class="bi bi-x-lg text-xl text-slate-700"></i></button>
        </div>
