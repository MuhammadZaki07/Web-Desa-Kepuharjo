...
<nav x-data="{ sidebarOpen: false, searchOpen: false }"
    ...
    <div class="hidden lg:flex items-center gap-10 justify-center flex-1">
        @foreach ($navItems as $item)
            @if (isset($item['dropdown']))
                @php
                    $activeSubLabel = $item['label'];
                    foreach ($item['dropdown'] as $subItem) {
                        if (request()->is(ltrim($subItem['href'], '/'))) {
                            $activeSubLabel = $subItem['label'];
                            break;
                        }
                    }
                @endphp

                <div x-data="{ open: false, selected: '{{ $activeSubLabel }}' }" x-init="selected = '{{ $activeSubLabel }}'"
                    ...
                    <div x-show="open" x-transition @mouseenter="open = true" @mouseleave="open = false"
                        ...
                        @foreach ($item['dropdown'] as $subItem)
                            <a href="{{ $subItem['href'] }}"
                               @click="selected = '{{ $subItem['label'] }}'; open = false"
                               class="block px-4 py-2 hover:bg-green-50 hover:text-green-600 text-slate-700
            ...
            @else
                <a href="{{ $item['href'] }}"
                   class="transition font-bold
                      {{ Str::startsWith(request()->path(), ltrim($item['href'], '/')) ? 'text-green-700 border-b-2 border-green-600 pb-1' : 'hover:text-green-700' }}">
                    {{ $item['label'] }}
                </a>
            @endif
        @endforeach
    ...
    <div class="relative">
        <div class="flex items-center gap-4 text-green-700">
            ...
            @auth
                <a href="/admin" class="text-md underline font-medium text-green-700">Dashboard Admin</a>
            @else
                <a href="/auth"><i class="bi bi-box-arrow-in-right cursor-pointer text-xl"></i></a>
            @endauth
        </div>
    ...
    <div x-show="sidebarOpen" x-transition:enter="transition duration-300"
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <button @click="sidebarOpen = false" class="cursor-pointer"><i class="bi bi-x-lg text-xl text-slate-700"></i></button>
        </div>
                ...
                @if (isset($item['dropdown']))
                    <div x-data="{ open: false }">
                        <button @click="open = !open"
                                class="flex justify-between cursor-pointer items-center w-full px-2 py-2 rounded text-left text-green-700 font-semibold hover:bg-gray-100">
                        ...
                        <div x-show="open" x-transition class="pl-4">
                            @foreach ($item['dropdown'] as $subItem)
                                <a href="{{ $subItem['href'] }}"
                                   class="block py-2 text-slate-700 hover:text-green-700">{{ $subItem['label'] }}</a>
                            @endforeach
                        </div>
                    </div>
                @else
